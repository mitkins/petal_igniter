defmodule PetalIgniter.Igniter.Templates do
  ...
  def add_warnings_for_missing_deps(igniter, base_module, deps, type \\ :source_folder) do
    deps
    |> Enum.reduce(igniter, fn {module, _file}, acc_igniter ->
      component_file =
        PetalIgniter.Igniter.Module.proper_location(acc_igniter, base_module, module, type)

      if !Igniter.exists?(acc_igniter, component_file) do
        Igniter.add_warning(
          acc_igniter,
          "Missing dependency file '#{component_file}'"
        )
      else
        acc_igniter
      end

defmodule <%= @module_prefix %>.Menu do
  use Phoenix.Component, global_prefixes: ~w(x-)
  import <%= @module_prefix %>.Link
  ...
  def vertical_menu_item(%{menu_items: _} = assigns) do
    ...
    >
      <button
        type="button"
        class={menu_item_classes(@current_page, @name)}
        {js_attributes("button", %{submenu_id: @submenu_id, icon_id: @icon_id})}
      >
        <.menu_icon icon={@icon} />
  ...
  defp js_attributes("button", _args) do
    %{
      "@click.prevent": "open = !open"
    }
  end

  defp js_attributes("icon", %{class: class}) do
    %{
      class: class,
  ...
  defp js_attributes("button", %{submenu_id: submenu_id, icon_id: icon_id}) do
    click =
      JS.toggle(
        to: "##{submenu_id}",
        display: "block"

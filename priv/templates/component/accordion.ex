defmodule <%= @module_prefix %>.Accordion do
  use Phoenix.Component
  <%= if @js_lib == "live_view_js" do %>alias Phoenix.LiveView.JS<% end %>
  ...
  def accordion(assigns) do
    ...
    <div
      ...
      {@rest}
      {js_attributes("container", @container_id, nil, nil, @open_index, @variant)}
    >
      <%%= for {current_item, i} <- Enum.with_index(@item) do %>
        <%% is_open = i == @open_index %>
        <div
          {js_attributes("item", @container_id, i, length(@item), is_open, @variant)}
          data-i={i}
        ...
        >
          <h2 id={content_panel_header_id(@container_id, i)}>
            <button
              type="button"
              {js_attributes("button", @container_id, i, length(@item), is_open, @variant)}
              class={
                if @variant == "ghost" do
              ...
              <%%= if @variant == "ghost" do %>
                <span class="pc-accordion-item__icon-container--ghost">
                  <.icon
                    ...
                    <%= if @js_lib == "alpine_js" do %>x-cloak<% else %>data-js-loading<% end %>
                    {js_attributes("icon", @container_id, i, length(@item), is_open, @variant)}
                  />
                  <.icon
                    name="hero-minus-mini"
                    class="pc-accordion-item__minus"
                    <%= if @js_lib == "alpine_js" do %>x-cloak<% else %>data-js-loading<% end %>
                    {js_attributes("icon_minus", @container_id, i, length(@item), is_open, @variant)}
                  />
                </span>
              <%% else %>
                <.icon
          ...
          <div
            {js_attributes("content_container", @container_id, i, length(@item), is_open, @variant)}
  ...
  <%= if @js_lib == "alpine_js" do %>
  defp js_attributes(type, container_id, i, l, open, variant) do
    case type do
      "container" ->

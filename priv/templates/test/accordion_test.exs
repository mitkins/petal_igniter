defmodule <%= @module_prefix %>.AccordionTest do
  use ComponentCase
  import <%= @module_prefix %>.Accordion
  ...
  test "accordion with open_index" do
    ...
    <%= if @js_lib == "alpine_js" do %>assert Regex.match?(~r{id="acc-content-panel-test_accordion-1".*?x-show="expanded"}s, html)<% end %>
  end
end
